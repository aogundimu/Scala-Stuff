...
class FunSetSuite extends FunSuite {
  ...
  test("diff contains elements in one set but not in the other") {
    new TestSets {
      val s = diff(s1, s2)
      val sd = diff(s2, s3)
      assert(contains(s, 1), "diff s1 and s2")
      assert(contains(sd, 2), "diff s2 and s3")
      assert(!contains(s, 2), "diff s1 and s2")
      assert(!contains(sd,3), "diff s2 and s3")
    }
  }

  test("filter a subset from a set of elements that satisfy a condition") {
    new TestSets {
      val s = union(s1, s2)
      assert( contains( filter(s, s1), 1), "filter s1")
      assert( contains( filter(s, s2), 2), "filter s2")
      assert( !contains( filter(s, s3), 3), "filter s3")
    }
  }
