...
object Huffman {
   ...
   */
   ...
   def until[Z](singleton: Z => Boolean, combine: Z => Z)(data: Z): Z = {
    if (singleton(data))
        data
    else
        until(singleton, combine)(combine(data))
   }
   ...
   */
    ...
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
       def stepThru(remaining: CodeTree, bits: List[Bit]): List[Char] = remaining match {
          case Leaf(c, _) if bits.isEmpty => List(c)
          case Leaf(c, _) => c :: stepThru(tree, bits)
          case Fork(left, right, _, _) if bits.head == 0 => stepThru(left, bits.tail)
          case Fork(left, right, _, _) => stepThru(right, bits.tail)
       }
   ...
   */
    ...
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def findVal(tree:  CodeTree)(c: Char): List[Bit] = tree match {
        case Leaf(_, _) => List()
        case Fork(left, right, _, _) if chars(left).contains(c) => 0 :: findVal(left)(c)
        case Fork(left, right, _, _) => 1 :: findVal(right)(c)
      }

      text flatMap findVal(tree)
    }
