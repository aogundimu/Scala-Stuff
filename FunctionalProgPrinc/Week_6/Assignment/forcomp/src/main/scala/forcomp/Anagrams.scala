...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(z => z).mapValues(z => z.length).toList.sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsHelper(oc: Occurrences): List[Sentence] = {
      if (oc.isEmpty) {
        List(Nil)
      } else {
        ...
        for (i <- combs if dictionaryByOccurrences.keySet(i);
             ...
             s <- sentenceAnagramsHelper(subtract(oc, i))) yield {j :: s}
      }
    }
